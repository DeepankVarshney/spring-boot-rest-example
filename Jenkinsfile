        ...
        stage("health-check"){
            steps{
                script{
                    ...
                    // while true; do
                    //         count=0
                    //         echo $(aws elbv2 describe-target-health  --target-group-arn arn:aws:elasticloadbalancing:us-east-1:159714198409:targetgroup/jenkins-test/236f8f6029b6913a --region $reg | grep "State" | cut -d':' -f 2 | cut -d'"' -f 2) > /home/ubuntu/state_check.txt
                    //         sed -i 's/ /\n/g' /home/ubuntu/state_check.txt
